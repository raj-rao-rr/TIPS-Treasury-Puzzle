...
for row = 1:T1
    ...
    maturity_match = find(days(abs(TIPS{row, 'Maturity'} - ...  
    ...
    for i = 1:size(bond_match, 1)
        ...
        if ismember(col_select, col_list)
            ...
            % we select the Treasury prices that correspond with our assigned CUSIP 
            % to the issue date of the matching TIPS
            treasury_prices = PRICE_T{PRICE_T.Dates >= bond_issue_date, col_select};
